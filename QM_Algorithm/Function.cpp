...
{
	vector<string> minimized; //changed
	auto it2 = EPIS.begin();
	for (auto it = copy_EPIS.begin(); it != copy_EPIS.end(); it++)
	{
		minimized.push_back(it2->first);
		//minimized += '+';
		it2++;
	...
	while (!temp_map.empty())
	{
		...
		for (auto it = binary_rep_mins.begin(); it != binary_rep_mins.end(); it++)
		{
			if (it->second.size() > size_row)
			{
				size_row = it->second.size();
				key_row = it->first;
			}
		}

		string temp_minimized = "";
		for (int i = 0; i < key_row.size(); i++)
		{
			if (key_row[i] == '1')
			{
				temp_minimized += literals[i];
			}
			else if (key_row[i] == '0')
			{
				temp_minimized += literals[i];
		...
		minimized.push_back(temp_minimized);

		for (auto x : binary_rep_mins[key_row]) {
			temp_map.erase(x);
		}
		binary_rep_mins.erase(key_row);  //changed 
	}
